...
### COPYRIGHT:
### 
### (C) COPYRIGHT International Business Machines Corp. 2005-2011
### All Rights Reserved
...
### SYNOPSIS:
###
###  trapit [-d] -e Error... -f File... [-i Interval] [-t Trigger]...
###
###         -d: Turn on debugging output for trapit itself
...
###         -v: Turn on verbose trapit output (debugging)
###
###
### DESCRIPTION:
###
...
### EXAMPLES:
###
### 1. To turn off WebSphere MQ tracing when an FFST appears with errors
...
### the 'true' command to indicate success.
  ...
  if [ -z "$ERRORS" ] ; then
    printf "trapit: Please give an error on which to search\n" >&2
    PrintSyntax && exit 1
  fi

  if [ -z "$FILES" ] ; then
    printf "trapit: Please give one or more files to search\n" >&2
    PrintSyntax && exit 1
  fi

  expr ${INTERVAL:=5} + 0 1>/dev/null 2>&1 || {
    printf "trapit: The scan interval must be a number\n" >&2
    PrintSyntax && exit 1
  }

  TRIGGERS="${TRIGGERS:=:};exec true"

  Verbose "Running trapit 4.0"
